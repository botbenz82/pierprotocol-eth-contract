...
contract PierStaking is Ownable, IPierStaking {
    ...
    function configureUnstakingDuration(uint256 newDuration) external override onlyOwner {
        unstakeDuration = newDuration;
    }

    function updatePool() external override onlyOwner {
    ...
    function urgentWithdrawUnclaimedRewards() external onlyOwner {
    ...
    function checkClaimableRewards(address _staker) external view returns (uint256) {
    ...
    function checkClaimableUnstakedToken(address _staker) external view returns (uint256) {
        UserInfo storage user = userInfo[_staker];
    ...
    function checkPendingUnstakeAmount(address _staker) external view returns (uint256) {
